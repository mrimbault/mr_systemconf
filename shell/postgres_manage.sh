...
export PGMANAGE="{{ git_dir }}/postgres-manage/postgres.pl"
export pgtools="{{ git_dir }}/postgres-manage/"
export pgsrc="{{ git_dir }}/postgresql/"
# FIXME requires a proper install directory
export postgres_manage="{{ home_dir }}/tools/postgres-manage/postgres-manage.conf"
...
alias pgpid="ps aux | ag 'postgres: \w+ \w+ .*(\(\d+\)|\[local\])' | head -n1 | awk '{print \$2}'"
...
alias pgctags_dualdir="ctags -R --exclude '.git' . {{ git_dir }}/postgresql"
